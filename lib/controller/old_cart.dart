...
class CartController extends GetxController implements GetxService{
  ...
  Future<Response> addToCart({ required int productId,required int variantId,required int quantity, bool fromProductDetail = false,bool fromHome = false}) {
  ...
  deleteCart({required int i ,required int index , bool fromView = true }) async{
    ...
    apiClient.putAPI(ApiProvider.deleteCart, {"product_id":cartModel.productId,"product_verient_id":cartModel.productVerientId,}).then((value) => {
      ...
      if (index < vendorProducts[i].cartProducts!.length) {
        vendorProducts[i].cartProducts![index].isDeleting = false,
        update(),
      }
    });
